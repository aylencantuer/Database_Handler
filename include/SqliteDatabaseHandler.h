...
private:
    ...
    int columnCount;
    std::shared_ptr<IniReader> iniReader;  // Corregido: uso de camelCase para el nombre
...
public:
    SqliteDatabaseHandler(const std::string& dbPath) : iniReader(std::make_shared<IniReader>())
    {
        this->openDb(dbPath);
    }
    ...
    void setConfFile(const std::string& confFilePath)
    {
        iniReader->open(confFilePath);  // Usar confFilePath en lugar de un nombre fijo
    ...
    Row fetch()
    {
        ...
        if (step == SQLITE_ROW) {
            ...
            {
                columnName = std::string(reinterpret_cast<const char*>(sqlite3_column_name(stmt, i)));
                columnValue = std::string(reinterpret_cast<const char*>(sqlite3_column_text(stmt, i)));
